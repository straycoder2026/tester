...
flowchart LR
	subgraph SITE[Pharmacy Client Site]
		CLIENT[PropelRx Desktop Client]
		PRINT[Label/Print target - verify]
	end

	subgraph DC[Hosted/Data Centre - verify]
		APP[Application runtime services - verify]
		WORKER[MessageOutQueue worker instance(s) - verify]
		SQL[(SQL Server)]
	end

	EXT1[Diem service - verify]
	EXT2[PIT/BC/PharmaNet - verify]
	...
	WORKER --> EXT1
	APP --> EXT2
	APP -. unproven direct path from analyzed workflows .-> EXT3
```
...
- [CODE-PROVEN] `Calls_cycle = 1 + 2P + 3R + C + U + M + D + 2G + X`; structural upper bound `1 + 5P + 3R + C + M + 2G` with `U<=P`, `D<=P`, `X<=P`.
...
- [CODE-PROVEN] `CancelOkClick=true` keeps modal open, enabling retry.
...
|---|---|---|---|---|---|---|---|---|
| F-001 | Queue return-value defect (`always true`) | [CODE-PROVEN] | High | High | Queue | False-success control flow | `MessageOutQueueService.cs:66-158`; `MessageOutQueueProcess.cs:52-55` | regression tests |
| F-002 | Diem success-list omission risk | [CODE-PROVEN] | High | High | Queue | Success result not accumulated before save | `MessageOutQueueService.cs:935-977` | integration send-result tests |
...
| F-006 | RxDetail lock/hydration cost risk | [CODE-PROVEN] | High | Medium | RxDetail | per-Rx lock path + full hydration branches | `RxDetailViewModel.cs:3206-3222`, `:11230`; `ConcurrencyService.cs:356-370` | lock duration and SQL count |
| F-007 | 67-result-set payload can increase load latency | [CODE-PROVEN]/[RUNTIME-VERIFY] | High | Medium | RxDetail | broad multi-result contract | `RxDetailDAO.cs:31-107`, `:251-320` | XE/QueryStore p95/p99, reads/rows |
...
| F-009 | ProcessNewRx partial-success + retry amplification | [CODE-PROVEN]/[RUNTIME-VERIFY] | High | Medium | ProcessNewRx | prior commits remain; retry path open | `ProcessNewRxViewModel.cs:1778-1813`; `BaseModalViewModel.cs:42-47` | duplicate artifact audit after retry |
| F-010 | Print/adjudication direct reachability unknown for analyzed create path | [INFERRED]/[RUNTIME-VERIFY] | Medium | Low-Medium | ProcessNewRx | method existence without direct static chain proof | `PrescriptionDAO.cs:2027-2054`; phase4 reachability notes | runtime trace correlation |
...
|---|---|---|---|---|---|---|---|---|---|---|
| RQ-001 | F-001,F-002,F-003 | Correct queue worker logic defects (return semantics, success-list accumulation, safe lookup) | Reliability correctness in outbound processing | S | Low-Med | none | unit + integration tests for queue result paths | feature-flag or staged service deploy | revert worker code change | `Shared.RxCore/Services/SendCommunication/MessageOutQueueService.cs` |
...
1. **Fix confirmed queue defects D-001/D-002/D-003** in `MessageOutQueueService` with targeted tests.
...
6. **Protect against repeated submission** where evidence supports safe idempotency checks.
...
Privacy-safe requirement: log only correlation IDs, timings, counts, statuses, and hashed/non-PHI identifiers. Do not log patient or prescription content.
...
**CareRx can approve now**
1. Stage 0 instrumentation plan.
2. Stage 1 confirmed queue defect fixes (D-001/D-002/D-003).
3. Targeted runtime measurement program for RxDetail and ProcessNewRx risk validation.

**Requires runtime evidence before approval**
